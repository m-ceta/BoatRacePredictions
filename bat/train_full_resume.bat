...
if exist "%PIPELINE_STATE_DIR%\02_zip_update_local.done" (
    echo [2/5] zip_update_local skipped: 02_zip_update_local.done exists
) else (
...
if exist "%PIPELINE_STATE_DIR%\03_build.done" (
    echo [3/5] build skipped: 03_build.done exists
) else (
...
if exist "%PIPELINE_STATE_DIR%\04_train.done" (
    echo [4/5] train skipped: 04_train.done exists
) else (
    call "%~dp0_common.bat" :run_step "[4/5] train" boatrace-train --config configs/train.yaml --resume
    if errorlevel 1 exit /b %errorlevel%
    type nul > "%PIPELINE_STATE_DIR%\04_train.done"
)

if exist "%PIPELINE_STATE_DIR%\05_zip_upload.done" (
    echo [5/5] zip_upload skipped: 05_zip_upload.done exists
) else (
