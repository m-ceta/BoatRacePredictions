...
if errorlevel 1 exit /b %errorlevel%
call "%~dp0_common.bat" :run_step "[4/6] train trifecta v2/v3" boatrace-train-trifecta-v2 --config configs/train.yaml --max-races %MAX_RACES% --eval-max-races %EVAL_MAX_RACES% --optimize-rerank --reset-rerank-optimization
if errorlevel 1 exit /b %errorlevel%
