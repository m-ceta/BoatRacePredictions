...
DEFAULT_BRP_DRIVE_FILE_URL = "https://drive.google.com/file/d/14w8W6xqi-NmnePs7waYhrUrxYD378YHq/view?usp=drive_link"
DEFAULT_ROWDATA_DRIVE_FILE_URL = "https://drive.google.com/file/d/1mtjumyk9k43UlGa7c2URfAZmUFgt_9En/view?usp=drive_link"
...
def download_and_restore_packages(
    project_root: Path,
    brp_drive_file_url: str = DEFAULT_BRP_DRIVE_FILE_URL,
    rowdata_drive_file_url: str = DEFAULT_ROWDATA_DRIVE_FILE_URL,
    brp_zip_name: str = "brp.zip",
    rowdata_zip_name: str = "rowdata.zip",
) -> DriveRestoreReport:
    root = project_root.resolve()
    rowdata_zip = root / rowdata_zip_name
    brp_zip = root / brp_zip_name

    _download_drive_file(extract_drive_file_id(rowdata_drive_file_url), rowdata_zip)
    _download_drive_file(extract_drive_file_id(brp_drive_file_url), brp_zip)

    restored_targets: list[str] = []
    with TemporaryDirectory(dir=root) as temp_dir_str:
        temp_dir = Path(temp_dir_str)
        rowdata_extract_root = _extract_zip_to_temp(rowdata_zip, temp_dir)
        _replace_directory_from_extracted(rowdata_extract_root, "rowdata", root / "rowdata")
        restored_targets.append("rowdata")

        brp_extract_root = _extract_zip_to_temp(brp_zip, temp_dir)
        _replace_directory_from_extracted(brp_extract_root, "data", root / "data")
        restored_targets.append("data")
        _replace_directory_from_extracted(brp_extract_root, "artifacts", root / "artifacts")
